...
function makeEquation(score: number, id: number): Equation {
  ...
  const duration = Math.max(4, 9 - Math.floor(score / 5));
  const left = 10 + Math.random() * 55;
...
export default function MattePage() {
  ...
  const gameRef = useRef<GameState>('idle');
  ...
  const handleAnswer = useCallback((answer: number) => {
    setEquation(prev => {
      ...
      if (correct) {
        ...
        setDotsPhase('split');
        setTimeout(() => setDotsPhase('merged'), 500);
        setTimeout(() => setDots(null), 1900);
      ...
      <div className="min-h-screen bg-gradient-to-br from-violet-50 to-purple-50">
        ...
        <div className="flex flex-col items-center justify-center min-h-[80vh] px-6 text-center">
          ...
          <p className="text-lg text-gray-500 mb-8 max-w-sm font-semibold">
            Tal ramlar ner — välj rätt svar innan det når botten!
          </p>
          ...
          <button
            onClick={startGame}
            className="px-12 py-5 rounded-3xl bg-gradient-to-r from-violet-500 to-purple-500 text-white font-black text-2xl shadow-lg hover:shadow-xl active:scale-95 transition-all"
          >
            Starta! 🚀
          </button>
        </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-violet-50 to-purple-50">
        ...
        <div className="flex flex-col items-center justify-center min-h-[80vh] px-6 text-center">
          ...
          <div className="flex gap-4 mt-4">
            <button
              onClick={startGame}
              className="px-8 py-4 rounded-3xl bg-gradient-to-r from-violet-500 to-purple-500 text-white font-black text-xl shadow-lg hover:shadow-xl active:scale-95 transition-all"
            >
              Spela igen 🔄
            </button>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-violet-50 to-purple-50 overflow-hidden relative select-none">
      ...
      {/* Falling equation area */}
      <div className="absolute top-16 left-0 right-0 bottom-32 overflow-hidden pointer-events-none">
        {equation && (
          <FallingEquation
            key={equation.id}
            equation={equation}
      ...
      {dots && (
        <div className="fixed bottom-36 left-0 right-0 flex justify-center pointer-events-none z-20 px-4">
          <div className="bg-white/95 rounded-3xl px-6 py-4 shadow-xl border border-violet-100 flex items-center gap-4 max-w-xs">
            <div className={`flex flex-wrap gap-1.5 max-w-[70px] transition-all duration-500 ${dotsPhase === 'merged' ? 'opacity-0 scale-50' : 'opacity-100 scale-100'}`}>
            ...
            <div className={`flex flex-col items-center transition-all duration-500 ${dotsPhase === 'merged' ? 'opacity-100 scale-110' : 'opacity-0 scale-50'}`}>
